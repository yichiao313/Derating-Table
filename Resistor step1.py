...
try:
    ...
    # 篩選 B64 零件
    filtered = df_bom[df_bom['Child Part'].astype(str).str.startswith("B64")]
    print(f"🔎 找到 {len(filtered)} 筆 B64 零件")
    ...
    # 寫入 Derating Table Resistor Sheet
    existing_df = pd.read_excel(derating_table_path, sheet_name="Resistor")
    ...
    with pd.ExcelWriter(derating_table_path, mode='a', engine='openpyxl', if_sheet_exists='overlay') as writer:
        result.to_excel(writer, sheet_name="Resistor", index=False, header=False, startrow=startrow)

    print("✅ 寫入 Derating 表成功")

    # 額外備份輸出
    output_path = "step2_Resistors.xlsx"
    result.to_excel(output_path, index=False)
    print(f"✅ 已另存 B64 篩選資料為：{output_path}")
